...
// Get transportation ID from URL with validation
$transport_id = isset($_GET['id']) ? mysqli_real_escape_string($conn, $_GET['id']) : '';
        ...
        <div class="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-lg">
          ...
          <form action="" method="POST" enctype="multipart/form-data">
            <select id="category" name="category" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
              ...
              <option value="luxury" <?= strtolower($transport['category']) === 'luxury' ? 'selected' : '' ?>>Luxury</option>
              <option value="vip" <?= strtolower($transport['category']) === 'vip' ? 'selected' : '' ?>>VIP</option>
              <option value="shared" <?= strtolower($transport['category']) === 'shared' ? 'selected' : '' ?>>Shared</option>
            </select>
            ...
            <div style="display:none">
              <?php
              echo "Current category from database: " . $transport['category'];
              var_dump($transport);
              ?>
            </div>
